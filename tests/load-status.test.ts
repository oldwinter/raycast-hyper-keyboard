...
describe("load status copy", () => {
  ...
  it("names an explicit missing Canvas path without inventing a personal vault", () => {
    expect(missingCanvasFileWarning("/tmp/map.canvas")).toBe("Canvas file not found: /tmp/map.canvas");
    expect(missingCanvasFileWarning("/tmp/map.canvas")).not.toMatch(/oldwinter-notes/);
  });
});
